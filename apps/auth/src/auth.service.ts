...
export class AuthService {
  ...
  async googleAuth(request: CreateUserRequest) {
    try {
      ...
      if (!userExists) {
        ...
        return await this.signToken(user._id, user.email, user.firstName, user.lastName, user.phoneNumber)
      }

      return await this.signToken(userExists._id, userExists.email, userExists.firstName, userExists.lastName, userExists.phoneNumber)
    }
    catch (err) {
      throw err
    }
  ...
  async signUp(request: CreateUserRequest) {
    try {
      ...
      const newUser = await this.authRepo.create({ ...request, password: hash })
      return newUser
    }
    catch (err) {
      throw err
  ...
  async changePassword(userId: any, password: any) {
    ...
    const updateUser = await this.authRepo.findOneAndUpdate({ _id: userId }, { password: hash })
  ...
  async signToken(
    ...
    firstName: string,
    lastName: string,
    phoneNumber: string): Promise<{ accessToken: string }> {
    const payload = {
      userId,
      email,
      firstName,
    ...
    const token = await this.jwtSvc.signAsync(payload, { expiresIn: `${expiresIn}m`, secret: secret })
